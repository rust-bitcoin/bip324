...
impl PacketWriter {
    ...
    /// - `aad`       - Optional authentication for the peer, currently only used for the first round of messages.
    /// - `packet`    - Buffer to write backet bytes too which must have enough capacity for the plaintext length in bytes + 20.
...
impl PacketHandler {
    ...
    /// # Arguments
    ///
    /// - `plaintext` - The Bitcoin P2P protocol message to send.
    /// - `aad`       - Optional authentication for the peer, currently only used for the first round of messages.
    /// - `decoy`     - Should the peer ignore this message.
    ///
    /// # Returns
    ...
    pub fn prepare_packet_with_alloc(
        ...
        self.packet_writer
            .prepare_packet_with_alloc(plaintext, aad, decoy)
    }
...
mod tests {
    ...
    fn test_authenticated_garbage() {
        ...
        let enc_packet = alice_packet_handler
            .prepare_packet_with_alloc(&VERSION_CONTENT, Some(&auth_garbage), false)
            .unwrap();
        let _ = bob_packet_handler
            .decrypt_contents_with_alloc(&enc_packet[LENGTH_BYTES..], Some(&auth_garbage))
            .unwrap();
    }
    ...
    fn test_vector_1() {
        ...
        let enc = alice_packet_handler
            .prepare_packet_with_alloc(&first, None, false)
            .unwrap();
        let dec_packet = bob_packet_handler
    ...
    fn test_vector_2() {
        ...
        for _ in 0..999 {
            alice_packet_handler
                .prepare_packet_with_alloc(&Vec::new(), None, false)
                .unwrap();
        }
        let enc = alice_packet_handler
            .prepare_packet_with_alloc(&contents, None, false)
    ...
    fn test_vector_3() {
        ...
        let auth = alice_packet_handler
            .prepare_packet_with_alloc(&contents, Some(&aad), false)
            .unwrap();
    ...
    fn test_vector_4() {
        ...
        for _ in 0..223 {
            alice_packet_handler
                .prepare_packet_with_alloc(&Vec::new(), None, true)
                .unwrap();
        }
        let enc = alice_packet_handler
            .prepare_packet_with_alloc(&contents, None, true)
            .unwrap();
        assert_eq!(
