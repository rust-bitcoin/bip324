...
impl PacketReader {
    ...
    pub fn decrypt_contents(
        ...
        let (msg, tag) = contents.split_at_mut(contents_len - 16);
        self.packet_decoding_cipher
...
impl PacketHandler {
    ...
    pub fn receive_v2_packets(
        ...
        while start_index.is_some() {
            let (message, index) =
                self.decode_packet_from_len(&mut ciphertext, &auth, start_index.unwrap())?;
            start_index = index;
    ...
    fn decode_packet_from_len(
        ...
        let content_len = u32::from_le_bytes(content_slice);
        let content_start_index = start_index + 3;
        let content_end_index = start_index + 3 + content_len as usize;

        // TODO: where is this 1 from?
        let len_with_tag = 1 + content_len + 16;

        // Calculate next message index.
        let mut next_content: Option<usize> = None;
        if len_with_tag > ciphertext.len() as u32 {
            return Err(Error::OutOfSync);
        }
        if start_index as u32 + len_with_tag + 3 < ciphertext.len() as u32 {
            next_content = Some((start_index as u32 + len_with_tag + 3) as usize);
        }

        let mut tag = [0u8; 16];
        tag.copy_from_slice(&ciphertext[content_end_index..content_end_index + 16]);
        self.packet_reader.packet_decoding_cipher.decrypt(
            auth,
            &mut ciphertext[content_start_index..content_end_index],
            tag,
        )?;
        let header = *ciphertext[content_start_index..content_end_index]
            .first()
            .expect("All contents should include a header.");
        if header.eq(&DECOY) {
            return Ok((None, next_content));
        }
        let message = ciphertext[content_start_index + 1..content_end_index].to_vec();
