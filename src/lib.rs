...
impl PacketHandler {
    fn new(session_keys: SessionKeyMaterial, role: HandshakeRole) -> Self {
        match role {
            HandshakeRole::Initiator => {
                ...
                PacketHandler { session_keys, role, length_encoding_cipher, length_decoding_cipher, packet_encoding_cipher, packet_decoding_cipher }
            },
    ...
    pub fn receive_v2_packet(&mut self, ciphertext: Vec<u8>, aad: Option<Vec<u8>>) -> Result<Vec<u8>, CipherError> {
        ...
        let aead = ciphertext[3..(aead_len as usize) + 3].to_vec();
...
pub fn initiator_complete_v2_handshake(message: Vec<u8>, init_handshake: InitiatorHandshake) -> Result<CompleteHandshake, HandshakeCompletionError> {
    ...
    match garbage_aad {
        Some((garbage, contents)) => {
            packet_handler.receive_v2_packet(contents, Some(garbage))
...
pub fn responder_complete_v2_handshake(message: Vec<u8>, responder_handshake: &mut ResponderHandshake) -> Result<(), HandshakeCompletionError> {
    ...
    responder_handshake.packet_handler.receive_v2_packet(garbage_auth, Some(responder_handshake.initiator_garbage.clone()))
                                    .map_err(|e| { HandshakeCompletionError::DecryptionError(e.to_string())})?;
    Ok(())
}
...
mod tests {
    ...
    fn test_packet_handler() {
        ...
        let dec = bob_packet_handler.receive_v2_packet(enc_packet, None).unwrap();
        ...
        let dec = alice_packet_handler.receive_v2_packet(enc_packet, None).unwrap();
        assert_eq!(message, dec);
    }
    ...
    fn test_fuzz_packets() {
        ...
        for _ in 0..REKEY_INTERVAL + 100 {
            ...
            let dec_packet = bob_packet_handler.receive_v2_packet(enc_packet, None).unwrap();
            assert_eq!(message, dec_packet);
            let message = gen_garbage(420);
            let enc_packet = bob_packet_handler.prepare_v2_packet(message.clone(), None, false).unwrap();
            let dec_packet = alice_packet_handler.receive_v2_packet(enc_packet, None).unwrap();
            assert_eq!(message, dec_packet);
        }
    }
    ...
    fn test_authenticated_garbage() {
        ...
        let dec_packet = bob_packet_handler.receive_v2_packet(enc_packet, Some(auth_garbage)).unwrap();
    }
    ...
    fn test_vector_1() {
        ...
        let dec_packet = bob_packet_handler.receive_v2_packet(enc, None).unwrap();
        assert_eq!(first, dec_packet);
    ...
    fn test_vector_2() {
        ...
        let mut bob_packet_handler = PacketHandler::new(session_keys, HandshakeRole::Initiator);
